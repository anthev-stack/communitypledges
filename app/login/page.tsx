...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (checkResponse.ok) {
        const { accountType } = await checkResponse.json()

        if (accountType === "discord") {
          setError(
            "It looks like you signed up with Discord. Please use the “Continue with Discord” button below to log in."
          )
          setLoading(false)
          return
        }
      ...
      if (result?.error) {
        setError(
          "Invalid email or password. If you signed up with Discord, please use the “Continue with Discord” button below."
        )
      } else {
  ...
  return (
    <MarketingAuthLayout
      ...
      subtitle={
        <>
          Or{" "}
          <Link href="/register" className="marketing-auth__inline-link">
      ...
      <form onSubmit={handleSubmit} className="marketing-auth__form">
        ...
        <div className="marketing-auth__field">
          <div className="flex items-center justify-between mb-1">
            ...
            <Link href="/forgot-password" className="marketing-auth__inline-link text-sm">
              Forgot password?
            </Link>
          </div>
          <input
